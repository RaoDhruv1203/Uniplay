...
(async () => {
  ...
  try {
    ...
    await page.locator('#browser-choice').selectOption('firefox');
    await page.locator('#browser-connect').click();
    await page.getByText('Connected · Firefox').waitFor();
    if ((await page.evaluate(() => window.downyt.state())).settings.cookiesBrowser !== 'firefox') throw new Error('Browser choice did not persist');
    await page.locator('#browser-clear').click();
    await page.getByText('Not connected', { exact: true }).waitFor();
    console.log('Opt-in local file/browser authentication controls passed');
