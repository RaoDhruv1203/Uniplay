...
(async () => {
  ...
  try {
    ...
    await page.frameLocator('#youtube-player').locator('video').waitFor({ timeout: 30000 });
    if ((await app.windows()).some(w => w.url().includes('pip.html'))) throw new Error('YouTube opened floating player before Float was clicked');
    ...
    await pip.locator('#player-notice.visible').waitFor();
    if (!await pip.locator('#player-notice').innerText().then(text => text.includes('YouTube playback must remain visible'))) throw new Error('YouTube audio explanation missing');
