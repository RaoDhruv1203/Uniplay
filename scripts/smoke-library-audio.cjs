...
(async () => {
  ...
  try {
    ...
    const restored = await app.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows().find(window => window.webContents.getURL().includes('pip.html')).getBounds());
    if (Math.abs(restored.width / restored.height - 16 / 9) > 0.03) throw new Error('Returning to video mode lost the player aspect ratio');
    await page.locator('[data-job-id="qa-video"] [data-action="play"]').click();
    await pip.locator('#audio-mode').click();
    await pip.locator('#pip.audio-mode').waitFor();
    await pip.waitForFunction(() => document.querySelector('#audio-cover').src.startsWith('file:') && document.querySelector('#audio-cover').naturalWidth > 0, null, { timeout: 30000 });
    if (!(await pip.locator('.audio-art').evaluate(element => getComputedStyle(element, '::after').backgroundImage.includes('gradient')))) throw new Error('Audio artwork vignette is missing');
    console.log('Saved video first frame became the compact audio artwork');
