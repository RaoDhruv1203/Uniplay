...
(async () => {
  ...
  if (!release) {
    release = await request('POST', `${api}/releases`, {
      ...
      body: `UNiPLAY for Windows. Download and run **UNiPLAY.exe**, or update from the bell in the app.\n\nNew in ${version}: anonymous-first YouTube analysis and direct playback for supported videos, with browser cookies optional; Live Studio scene resizing and crop controls, searchable system fonts, gradient text and backdrops, customizable LIVE badge, and supported website/YouTube media links; M3U8 playlist links in Live, including HLS master variants and GitHub file-page links; adjustable channel-logo grid tiles. Live Studio can use your own Cloudflare managed tunnel and hostname for remote sharing; the tunnel token is encrypted locally and not included in this release.\n\nA managed public link requires your own Cloudflare tunnel and hostname configured to route to http://localhost:7767. Account-free Quick Tunnels remain a testing option without an uptime guarantee. Not all YouTube or website videos offer a playable direct stream. The installer is not code-signed, so Windows may display an Unknown publisher warning.\n\nSHA-256 (UNiPLAY.exe): \`${digest}\``,
      draft: true,
      prerelease: false,
    });
